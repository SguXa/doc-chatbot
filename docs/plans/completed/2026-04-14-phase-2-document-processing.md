...
- [x] Confirm `documents` schema has NO `status`, `failed_at`, `error_code` columns (PRAGMA `table_info(documents)`)
- [x] Confirm `Document` data class has NO `status`, `failedAt`, `errorCode` fields
- [x] Confirm `UploadResult` has exactly two variants: `Created` and `Duplicate` (no `Failed`)
- [x] Confirm every failure-mode test asserts "no document row inserted" (ADR 0003 invariant)
- [x] Confirm startup orphan scan deletes only `*.tmp.*` files; no code path attempts to detect or heal orphan **final** artifacts (ADR 0004)
- [x] Grep backend source tree: no occurrences of `Authentication`, `BearerAuth`, `JWT`, `authenticate {`, `principal`, `Authorization`, `AUTH_DISABLED`, `DEV_BYPASS_AUTH`, `SKIP_AUTH` (ADR 0005 — auth is Phase 4 only)
- [x] Confirm startup WARN about unprotected admin routes fires in `MODE=full`/`MODE=admin`, does NOT fire in `MODE=client`
