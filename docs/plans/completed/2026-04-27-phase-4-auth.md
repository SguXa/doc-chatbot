...
- [x] Once all checkboxes above are green, move this plan to `docs/plans/completed/<today's date>-phase-4-auth.md` (use the date the phase actually finishes, in `YYYY-MM-DD` form, matching the convention of `2026-04-09-phase-1-project-foundation.md` etc.)
- [x] Verify the moved plan still renders correctly (no relative-path breakage to `docs/adr/`)
